...
export const combinedHash = (first: Buffer, second: Buffer): Buffer => {
    if (!first) {
        return second
    }
    if (!second) {
...
export const toNode = (index: number | BigNumber, account: string, amount: BigNumber): Buffer => {
...
export const getNextLayer = (elements: Buffer[]): Buffer[] => {
...
export const getRoot = (balances: { account: string; amount: BigNumber; index: number }[]): Buffer => {
    let nodes = balances
